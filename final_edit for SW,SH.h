...
void Sw(string source, string destination, string offset) { // sw x9,0(x2)
	...
	if (i == memory_address_values.end())
		...
		i->second[0] = to_string(source_value);
...
void SH(string source, string destination, string offset) { // sw x9,0(x2)
	...
	if (i == memory_address_values.end())
		memory_address_values.insert({ destination_value,{to_string(source_value),"0b" + tobinary(to_string(source_value)),"0x" + tohexa(to_string(source_value))} });
			...
			string value,binary_value;
			int position = stoi(offset) % 4;
			if (position == 0) value = to_string(source_value);
			else {
				i->second[0]=to_string(stoi(i->second[0])&((1 << 16) - 1));
				binary_value = decimal_to_binary(source_value) + decimal_to_binary(stoi(i->second[0]));
				value = to_string(binaryToDecimal(binary_value));
			}
			i->second[0] = value;
			i->second[1] = tobinary(value);
...
void SB(string source, string destination, string offset) { // sw x9,0(x2)
	...
	if (i == memory_address_values.end())
		...
		if (position == 0) value = to_string(source_value);
			...
			i->second[0] = to_string(stoi(i->second[0]) & ((1 << 8) - 1));
			binary_value = decimal_to_binary(source_value) + decimal_to_binary(stoi(i->second[0]));
			value = to_string(binaryToDecimal(binary_value));
		}
		else if (position == 2) {
			...
			i->second[0] = to_string(stoi(i->second[0]) & ((1 << 24) - 1));
			binary_value = decimal_to_binary(source_value) + decimal_to_binary(stoi(i->second[0]));
			value = to_string(binaryToDecimal(binary_value));
		}
		i->second[0] = value;
		i->second[1] = "0b"+tobinary(value);
		i->second[1] = "0x"+tohexa(i->second[0]);
...
void LH(string source, string destination, string offset) {
	...
	value = value & ((1 << 16) - 1);
	string binary_value;
	binary_value = tobinary(to_string(-value));
	value = binaryToDecimal(binary_value);
	auto it = registers.find(destination);
	it->second[0] = to_string(value);
	it->second[1] = "0b"+tobinary(to_string(value));
	it->second[2] = "0x"+tohexa(to_string(value));
	printMap();
}
